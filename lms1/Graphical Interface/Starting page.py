from barcode import *
from warning_email import warning_email_send
...
frame25 = tk.Frame(window)   #PhotoUpload
frame26 = tk.Frame(window)   #barcodeissue
...
for frame in (mainframe, issue, issue_confirmation, search, frame5, frame6, frame7, frame8, frame9, frame10, frame11, frame12, frame13, frame14, frame15, frame16, frame17, frame18, frame19, frame20, frame21, frame22, frame23, frame24, frame25, frame26):
...
txtsearch=Button(mainframe, text= "Photo", font=('arial', 20), bg="white", width= 10, height= 0, command=lambda: show_frame(frame25))
txtsearch.place(x= 1550, y=400)

frame1_insert = tk.Button(mainframe, text='ISSUE', fg='black', command=lambda: show_frame(issue), bg="white", padx=200, pady=20)
frame1_insert.place(x=150, y=750)

frame1_return = tk.Button(mainframe, text='RETURN',fg='black', command=lambda: returnf(), bg="white", padx=200, pady=20)
frame1_return.place(x=738, y=750)

frame1_extention = tk.Button(mainframe, text='EXTENTION',fg='black', command=lambda: extentionf(), bg="white", padx=200, pady=20)
...
frame1_record = tk.Button(mainframe, text='RECORDS',fg='black', command=lambda: [issued_records(),show_frame(frame7)], bg="white", padx=200, pady=20)
frame1_record.place(x=440, y=840)

frame1_stock = tk.Button(mainframe, text='STOCK',fg='black', command=lambda: show_frame(frame8), bg="white", padx=200, pady=20)
...
def search1():
   ...
   if not Search or not searchbar:
      ...
      if searched_name==0:
         ...
         amazon_button.place(x=500, y=500)
...
frame8_show = tk.Button(frame8, text='SHOW BOOKS', fg='black', command=lambda: show_frame(frame14), bg="white", padx=200, pady=20)
frame8_show.place(x=150, y=600)

frame8_add = tk.Button(frame8, text='ADD BOOKS',fg='black', command=lambda: show_frame(frame15), bg="white", padx=200, pady=20)
frame8_add.place(x=738, y=600)

frame8_update = tk.Button(frame8, text='UPDATE BOOKS',fg='black', command=lambda: show_frame(frame16), bg="white", padx=200, pady=20)
frame8_update.place(x=1300, y=600)

frame8_delete = tk.Button(frame8, text='DELETE BOOKS',fg='black', command=lambda: show_frame(frame17), bg="white", padx=200, pady=20)
frame8_delete.place(x=440, y=690)

frame8_studentlist = tk.Button(frame8, text='STUDENT LIST',fg='black', command=lambda: [all_students(), show_frame(frame18)], bg="white", padx=200, pady=20)
...
def get_value():
   ...
   book_data2=isbnsearch1(ISBN)
   addbooks(str(book_data2[0]),str(book_data2[1]),str(book_data2[2]),str(ISBN),str(bookquantity),str(book_data2[3][0]),str(barcode1))
...
lblbookcode = Label(addframe, bg="#C1C1C1", text="Bar Code:", font=("times new roman", 28, "bold"), padx=125, pady=0)
lblbookcode.grid(row=2, column=0, sticky=W)

txtbarcode=Entry(addframe, font=("times new roman", 20, "bold"), width= 13)
txtbarcode.grid(row=2, column=1)
...
"==================================================FRAME 25(PHOTO UPLOAD)=========================================================================================================================="

bg_photoupload = PhotoImage(file="Images/Illustration21.png")
photoupload_label=Label(frame25, image=bg_photoupload)
photoupload_label.place(x=0, y=0, relwidth=1, relheight=1)

frame25_backbutton = tk.Button(frame25, command=lambda: show_frame(mainframe), bg="white", padx=40, pady=20)
frame25_backbutton.place(x=10, y=10)
from tkinter import filedialog
from PIL import Image, ImageTk


def search2(image_text):
   searchbar = image_text
   if not searchbar:
      Label(search, text="Arey kuch toh enter karo", font=('Century 15 bold')).pack(pady=20)
   else:
      searched_name = search_book(searchbar)
      if searched_name == 0:
         Label(frame24, text="Hamare paas yeh book nahi nahi\nwould you like to search it up online?",
               font=('Century 15 bold')).pack(pady=300)

         google_button = Button(frame24, text="GOOGLE", fg="white", padx=200, pady=20, borderwidth=3, bg="#1E1E20",
                                command=lambda: online_search("Google", searchbar))
         google_button.place(x=150, y=500)

         amazon_button = Button(frame24, text="AMAZON", fg="white", padx=200, pady=20, borderwidth=3, bg="#1E1E20",
                                command=lambda: online_search("Amazon", searchbar))
         amazon_button.place(x=738, y=500)

         flipkart_button = Button(frame24, text="FLIPKART", fg="white", padx=200, pady=20, borderwidth=3, bg="#1E1E20",
                                  command=lambda: online_search("Flipkart", searchbar))
         flipkart_button.place(x=1000, y=500)
...
def upload_file():
   ...
   b2 = tk.Button(frame25, image=img)  # using Button
   ...
   search_button = Button(frame25, text="GO", fg="white", padx=85, pady=10, borderwidth=3, bg="#1E1E20",
                          command=lambda: search2(words_in_image))
   search_button.place(x=1485, y=575)

   language = 'en'

   """obj = gTTS(text=words_in_image, lang=language, slow=False)2
...
b1 = tk.Button(frame25, text='Upload File', width=20,command = lambda:upload_file())
b1.place(x=1500, y=400)
...
"==================================================FRAME 26(bar issue)=========================================================================================================================="
'''bg_barissue = PhotoImage(file="Images/Illustration21.png")
barissue_label=Label(frame26, image=bg_barissue)
barissue_label.place(x=0, y=0, relwidth=1, relheight=1)

barframe = LabelFrame(frame26, text="ISSUE DETAILS", bd=5, relief=RAISED, font=("arial italic", 20), padx=20,
                        bg="#DCDCDC")
barframe.place(x=600, y=250, width=700, height=500)


lblbar_issue = Label(barframe, bg="#C1C1C1", text="SCAN BARCODE", font=("times new roman", 28, "bold"),
                        padx=55, pady=0)
lblbar_issue.grid(row=2, column=0, sticky=W)

scan_button = Button(barframe, text="NEXT", fg="white", padx=85, pady=10, borderwidth=3, bg="#1E1E20",
                     command=lambda: barcodefinal())    #asdfghjklllllllllllllll
scan_button.grid(row=2, column=1)


next_button = Button(barframe, text="NEXT", fg="white", padx=85, pady=10, borderwidth=3, bg="#1E1E20",
                     command=lambda: show_frame(issue))
next_button.grid(row=4, column=1)

frame26_backbutton = tk.Button(frame26, command=lambda: show_frame(mainframe), bg="white", padx=40, pady=20)
frame26_backbutton.place(x=10, y=10)'''
